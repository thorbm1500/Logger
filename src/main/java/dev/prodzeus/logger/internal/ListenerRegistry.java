    ...
    public static void register(@NotNull final Listener listener) throws IllegalArgumentException {
        ...
        while (!methods.isEmpty()) {
            ...
            for (final Level level : levels) {
                //LOGGER.warn("Method: {}, Level: {}", method.getName(), level);
                if (!method.getDeclaringClass().equals(listener.getClass())
                    && !(method.getName().toLowerCase().contains("ongenericlogevent")
                         || method.getName().toLowerCase().contains(level.toString().toLowerCase()))) continue;
                synchronized (mutex) {
                    final HashMap<UUID, Consumer<Event>> map = sinks.getOrDefault(level, new HashMap<>());
                    map.put(listener.getUniqueId(), event -> {
                        try {
